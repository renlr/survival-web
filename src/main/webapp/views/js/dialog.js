...
(function() {
	...
	var X = function($, C, _) {
		...
		return I($.id).int(A)
	}, d = 4, _ = 999999, O = "temp_artDialog", S = 100000, D = "\xd7", W = "Loading..", L = 0, C = {}, R, e = [], P, N, U, M = !-[
			1, ], J = M
			&& ([ /MSIE (\d)\.0/i.exec(navigator.userAgent) ][0][1] == 6), B = function(
			_, $) {
		for ( var B = 0, A = _.length; B < A; B++) {
			$(_[B], B)
		}
	}, Z = function(B) {
		var _ = B ? B.document.documentElement : document.documentElement, $ = B ? B.document.body
				: document.body, A = _ || $;
		return {
		...
		var _ = G || window.event, B = this.data.box, H = this.data.moveTemp, F = Z(), I = this
				.security(B);
		C.box = this;
		this.zIndex();
		P = true;
		...
		var _ = B || window.event, E = R ? C.box.data.moveTemp : C.box.data.box, G = _.clientX, F = _.clientY, A = Z(), D = parseInt(C.left
				- C.x + G - C.pageLeft + A.left), $ = parseInt(C.top - C.y + F
				- C.pageTop + A.top);
		if (D > C.maxX) {
			D = C.maxX
		}
		...
		var _ = A || window.event, F = _.clientX, E = _.clientY, $ = C.width
				+ F - C.x + B.w, D = C.height + E - C.y + B.h;
		if ($ > 0) {
		...
		if (J) {
			C.box.data.selectMask.style.width = C.box.data.box.offsetWidth
					+ "px";
			C.box.data.selectMask.style.height = C.box.data.box.offsetHeight
					+ "px"
		}
		return false
	}, I = function(Q) {
		...
		var s = {
			...
			int : function($) {
				...
				s.content($.title, $.content, $.iframe).yesBtn($.yesFn,
						$.yesText).noBtn($.noFn, $.noText).closeBtn($.closeFn)
						.size($.width, $.height).align($.menuBtn, $.left,
								$.top, $.fixed);
				if ($.lock) {
					s.lock.show()
				}
			...
			align : function(I, E, K, G) {
				...
				if (I && I.getBoundingClientRect) {
					var _ = s.data.boxWidth / 2 - I.offsetWidth / 2, H = I.offsetHeight, F = I
							.getBoundingClientRect().left, D = I
			...
			lock : {
				show : function() {
					...
					B([ "DOMMouseScroll", "mousewheel", "scroll" ], function($,
							_) {
				...
				hide : function() {
					if (m.className.indexOf("ui_lock") > -1) {
						s.alpha(E, 1, function() {
							...
							B([ "DOMMouseScroll", "mousewheel", "scroll",
			...
			alpha : function(B, A, D) {
				...
				var _ = function() {
					A = A + $;
					B.filters ? B.filters.alpha.opacity = A
							: B.style.opacity = A;
					if (0 >= A || A >= C) {
						if (D) {
							D()
	...
	f(window, "load", function() {
		if (!N) {
			artDialog({
				id : O,
				style : "confirm alert error succeed",
				time : 10,
				lock : false
			}, function() {
			}, function() {
