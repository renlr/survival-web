 ...
 * resizing, rotating, flipping� Plus it let you do some really cool affects.
...
public class Image {
	...
	public void resize(int newWidth, int newHeight) {
		...
		for (int y = 0; y < oldHeight && newY < newHeight; y++) {
			...
			if (isNewImageHeightSmaller && heightCounter > 1) {
				heightCounter -= 1;
	...
	/**
	 * !!很耗时间，但缩放出来的图片效果好很多 大约是.resize方法的两倍时间 建议一般不要使用
	 * 
	 * @param newWidth
	 * @param newHeight
	 */
	public void resize2(int newWidth, int newHeight) {
		ColorModel dstCM = bufferedImage.getColorModel();
		BufferedImage dst = new BufferedImage(dstCM, dstCM.createCompatibleWritableRaster(newWidth, newHeight), dstCM.isAlphaPremultiplied(), null);
		java.awt.Image scaleImage = bufferedImage.getScaledInstance(newWidth, newHeight, java.awt.Image.SCALE_SMOOTH);
